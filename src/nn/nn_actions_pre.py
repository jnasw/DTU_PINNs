...
from src.nn.nn_dataset_pre import HDF5Dataset
from src.nn.nn_model import Net, Network, PinnA, FullyConnectedResNet, Kalm, ActNet
...
class NeuralNetworkActions():
    ...
    """
    def __init__(self, cfg, modelling_full, data_loader, *args): # The modelling equations are used, must be predefined, more choices to be added such as dynamic modelling
        self.cfg = cfg
        set_random_seeds(cfg.seed) # set all seeds
        self.device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')  

        self.modelling_full = modelling_full # The modelling equations are used, must be predefined, more choices to be added such as dynamic modelling
        self.data_loader = data_loader # The data loader is used to load the data
        self.input_dim = self.data_loader.input_dim # The input dimension is the number of input features
        self.output_dim = self.data_loader.output_dim # The output dimension is equal to the number of odes
    ...
    def define_nn_model(self):
        ...
        This function defines the neural network model
        """
        print("Selected deep learning model: ",self.cfg.nn.type, "with configs found in", self.cfg.nn.nn_yaml_file)
        nn_cfg = OmegaConf.load(self.cfg.nn.nn_yaml_file)
        # find the correct nn config based on self.cfg.nn.type
        for key in nn_cfg.keys():
            if key == self.cfg.nn.type:
                print(nn_cfg[key].items()) # print all the specific configs for the selected nn type
                for k, v in nn_cfg[key].items():
                    if k not in self.cfg.nn:
                        self.cfg.nn[k] = v
                break
        ...
        if self.cfg.nn.type == "KAN": # Static architecture of the neural network
            ...
            model = Kalm(self.input_dim, self.cfg.nn.hidden_dim, self.output_dim, self.cfg.nn.hidden_layers, self.cfg.nn.G, self.cfg.nn.k)
            # model.speed()
        elif self.cfg.nn.type == "actnet":
            model = ActNet(
                    input_dim=self.input_dim,
                    embed_dim= self.cfg.nn.hidden_dim,
                    num_layers=self.cfg.nn.hidden_layers,
        ...
        elif self.cfg.nn.type == "DynamicNN" or self.cfg.nn.type == "PinnB" or self.cfg.nn.type == " ": # Dynamic architecture of the neural network
            model = Network(self.input_dim, self.cfg.nn.hidden_dim, self.output_dim, self.cfg.nn.hidden_layers,self.cfg.nn.activation_function)
        ...
        if platform.system() == "Windows":
            print("⚠️ Warning: torch.compile() is not supported on Windows. Returning uncompiled model.")
            return model  # Return model as-is
        else:
            return torch.compile(model)  # Use torch.compile() on supported OS
    ...
    def forward_nn(self, time, no_time):
        ...
        if self.cfg.nn.type in ["DynamicNN", "PinnAA", "ResNet", "KAN", "actnet"]:
            return y_pred
        else:
    ...
    def derivative(self, y, t): # create graph, otherwise wrong output..
        """
    ...
    def calculate_autograd222(self, x_train): 
    #     """
    ...
    def calculate_from_ode(self, output, input):
        """
        This function calculates the output(dy/dt) of the synchronous machine model for the given input y
        """
        append_dim = self.input_dim - self.output_dim -1
        output = torch.cat((output, input[:, -append_dim:]), dim=1)
    ...
    def calculate_point_grad2(self, x_train, y_train):
        ...
        time_autograd22 = time.time() - start_time

        start_time = time.time()
        y_hat_del, dy_dt_del = self.calculate_autograd222(x_train) # calculate the output of the model and the derivative of the output
        time_autograd222 = time.time() - start_time

        start_time = time.time()
        y_hat_jvp , dy_dt_jvp = self.calculate_autograd(x_train)
        time_autograd = time.time() - start_time
        
        print(f"Time for calculate_autograd22: {time_autograd22:.4f} seconds")
        print(f"Time for calculate_autograd222: {time_autograd222:.4f} seconds")
        print(f"Time for calculate_autograd: {time_autograd:.4f} seconds")
        print("original",y_hat.shape, y_hat_del.shape, y_hat_jvp.shape)
        print(y_hat[2:3])
        print("old")
        print(y_hat_del[2:3])
        print("jvp")
        print(y_hat_jvp[2:3])
        #print("new",dy_dt.shape, dy_dt_del.shape, dy_dt_jvp.shape)
        print(dy_dt[2:3])
        print("old")
        print(dy_dt_del[2:3])
        print("jvp")
        print(dy_dt_jvp[2:3])
        """
        y_hat, dy_dt = self.calculate_autograd(x_train) # use jvp for calculating the output and the derivative of the output
        

        #ode
        if y_train is None: # collocation points
            y_processed = self.calculate_from_ode(y_hat,x_train)
            return dy_dt , y_processed
        else:
            y_processed = self.calculate_from_ode(y_train,x_train) # data points
    ...
    def folder_name_f2(self, cfg):
        ...
        if weight_data == weight_dt == 0:
            folder_name = "pinn_ic" if weight_pinn_ic > 0 else "pinn"
        elif weight_dt == weight_pinn == 0:
            folder_name = "data_ic" if weight_pinn_ic > 0 else "data"
        elif weight_data == 0:
            folder_name = "dt_pinn" if weight_pinn_ic == 0 else "pinn_ic"
        elif weight_dt > 0 and weight_pinn > 0:
            folder_name = "data_dt_pinn_ic" if weight_pinn_ic > 0 else "data_dt_pinn"
    ...
    def pinn_train(self, num_of_skip_data_points, num_of_skip_col_points, num_of_skip_val_points, wandb_run=None):
        """
        This function trains the neural network model

        Args:
            num_of_skip_data_points (int): number of data points to skip
            num_of_skip_col_points (int): number of collocation points to skip
            num_of_skip_val_points (int): number of validation points to skip
            wandb_run (wandb): wandb run object
        """      
        self.wandb_run = wandb_run  
        # folder to find the datasets
        folder_path = "./"+self.cfg.dirs.dataset_dir+"/" + self.cfg.model.model_flag + '/dataset_v' + str(self.cfg.dataset.number)
        
        # Load all data once into memory (if feasible)
        train_dataset = HDF5DatasetStatic(folder_path, 'train', step=num_of_skip_data_points)
        col_dataset = HDF5DatasetStatic(folder_path, 'train', data_name="col", step=num_of_skip_col_points)
        init_dataset = HDF5DatasetStatic(folder_path, 'train', data_name="data_init", step=1)
        val_dataset = HDF5DatasetStatic(folder_path, 'val', step=num_of_skip_val_points)


        print("Number of labeled training data:", len(train_dataset),
            "Number of collocation points:", len(col_dataset),
            "Number of collocation points (IC):", len(init_dataset),
            "Number of validation data:", len(val_dataset))
         
            # Create DataLoaders with multiple workers for efficiency
        batch_size = self.cfg.nn.batch_size if self.cfg.nn.batch_size != "None" else len(train_dataset)
        
        # Create DataLoaders with optimizations
        train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True)
        col_loader = DataLoader(col_dataset, batch_size=batch_size, shuffle=True)
        ...
        col_loader = DataLoader(
            col_dataset, batch_size=batch_size, shuffle=True, 
            num_workers=4, pin_memory=True, persistent_workers=True, prefetch_factor=2
        )
        ...
        folder_name=self.folder_name_f2(self.cfg) # folder to save the model
        ...
        if self.cfg.nn.batch_size == "None":
            x_train = train_dataset.x_data.requires_grad_(True)
            y_train = train_dataset.y_data
            x_train_col = col_dataset.x_data.requires_grad_(True)
            x_train_col_ic = init_dataset.x_data.requires_grad_(True)
            y_train_col_ic = init_dataset.y_data

        x_val, y_val = val_dataset.x_data.requires_grad_(True), val_dataset.y_data

        print("getting in training")
        for epoch in range(self.cfg.nn.num_epochs):
            self.model.train() # set the model to training mode
            if self.cfg.nn.batch_size != "None":
            # Iterate over batches of data, considering as basis the number of batches of train_loader
                for _ in range(len(train_loader)):  
                    x_batch, y_batch = next(iter(train_loader))
                    x_col_batch = next(iter(col_loader))
                    x_col_ic_batch, y_col_ic_batch = next(iter(init_loader))

                    
                    def closure():
                        output, dydt0, ode0 = self.calculate_point_grad2(x_batch.requires_grad_(True), y_batch) # calculate nn output and its gradient for the data points, and the ode solution for the target y_train
                        dydt1, ode1 = self.calculate_point_grad2(x_col_batch.requires_grad_(True), None) # calculate nn output gradient for the collocation points, and the ode solution for the nn output
                        output_col0 = self.forward_pass(x_col_ic_batch.requires_grad_(True)) # calculate the nn output for the collocation points with time 0
                        
                        # Data and Physics Losses for the data points
                        loss_data = self.criterion(output, y_batch) # calculate the data loss
                        loss_dt =torch.stack([self.criterion(dydt0[:, i], ode0[:, i]) for i in range(dydt0.shape[1])])
                        mean_loss_dt = loss_dt.mean() # Stack and then mean
                        # _, _ = self.calc_adapt_criterion_loss(x_col_batch, dydt1, ode1)   #Later development
                        # Physics Loss for the collocation points
                        loss_pinn = torch.stack([self.criterion(dydt1[:, i], ode1[:, i]) for i in range(dydt1.shape[1])])
                        mean_loss_pinn = loss_pinn.mean() # Stack and then mean
                        # Initial Condition Loss for the collocation points
                        ...
                        self.loss_total, self.loss_data, self.loss_dt, self.loss_pinn, self.loss_pinn_ic = loss_total, loss_data, mean_loss_dt, mean_loss_pinn, loss_pinn_ic
                    ...
                    # scaler.step(self.optimizer, closure=closure)
                    # scaler.update()
            ...
            mean_loss_val_physics = torch.stack([self.criterion(val_dydt0[:, i], val_ode0[:, i]) for i in range(val_dydt0.shape[1])]).mean()
            ...
            save_iteration = 500 if self.cfg.nn.optimizer == "LBFGS" else 10000 
            if (epoch + 1) % save_iteration == 0:
                
                name = f"{self.cfg.model.model_flag}{self.cfg.nn.type}_{self.cfg.time}_{epoch+1}_{len(train_dataset)}_{len(col_dataset)}_{len(val_dataset)}_{self.cfg.dataset.transform_input}_{self.weight_data}_{self.weight_dt}_{self.weight_pinn}_{self.weight_pinn_ic}_{self.cfg.nn.weighting.update_weight_method}.pth"
        ...
        if self.early_stopping.early_stop == True or (epoch + 1) % save_iteration != 0:
            name = f"{self.cfg.model.model_flag}{self.cfg.nn.type}_{self.cfg.time}_{epoch+1}_{len(train_dataset)}_{len(col_dataset)}_{len(val_dataset)}_{self.cfg.dataset.transform_input}_{self.weight_data}_{self.weight_dt}_{self.weight_pinn}_{self.weight_pinn_ic}_{self.cfg.nn.weighting.update_weight_method}.pth"
            self.save_model(os.path.join(folder_name, name))
        self.final_name = os.path.join(folder_name, f"{self.cfg.model.model_flag}{self.cfg.nn.type}_{self.cfg.time}_{epoch+1}_{len(train_dataset)}_{len(col_dataset)}_{len(val_dataset)}_{self.cfg.dataset.transform_input}_{self.weight_data}_{self.weight_dt}_{self.weight_pinn}_{self.weight_pinn_ic}_{self.cfg.nn.weighting.update_weight_method}")
    ...
    def pinn_train2(self, num_of_skip_data_points, num_of_skip_col_points, num_of_skip_val_points, wandb_run=None):
        ...
        for epoch in range(self.cfg.nn.num_epochs):
            ...
            if self.cfg.nn.batch_size != "None":
                ...
                for (x_batch, y_batch), x_col_batch, (x_ic_batch, y_ic_batch) in zip(train_loader, collocation_loader, col_ic_loader):
                    ...
                    def closure():
                        ...
                        loss_total, self.losses = self.weighting_scheme.compute_weighted_loss(loss_data, loss_dt, loss_pinn, loss_pinn_ic, epoch) #total_iteration_count + iteration_count
                        self.loss_data, self.loss_dt, self.loss_pinn, self.loss_pinn_ic = loss_data, mean_loss_dt, mean_loss_pinn, loss_pinn_ic
                ...
                def closure():
                    ...
                    loss_dt = torch.stack([self.criterion(dydt0[:, i], ode0[:, i]) for i in range(dydt0.shape[1])])#.mean() # Stack and then mean
                    mean_loss_dt =loss_dt.mean()
                    # _, _ = self.calc_adapt_criterion_loss(x_train_col, dydt1, ode1)   #Later development
                    loss_pinn = torch.stack([self.criterion(dydt1[:, i], ode1[:, i]) for i in range(dydt1.shape[1])]) # Stack and then mean
                    mean_loss_pinn =loss_pinn.mean()
                    loss_pinn_ic = self.criterion(output_col0, y_train_col_ic)
            ...
            if self.optimizer != "LBFGS":
                self.scheduler.step()
            """
            print("this is what I want, ", loss_total, loss_data)
            # Validation
            self.model.eval()
            ...
            if (epoch + 1) % save_iteration == 0:
                ...
                name = f"{self.cfg.model.model_flag}{self.cfg.nn.type}_{self.cfg.time}_{epoch+1}_{self.data_loader.training_shape}_{self.data_loader.training_col_shape}_{self.data_loader.validation_shape}_{self.cfg.dataset.transform_input}_{self.weight_data}_{self.weight_dt}_{self.weight_pinn}_{self.weight_pinn_ic}_{self.cfg.nn.weighting.update_weight_method}.pth"
        ...
        if self.early_stopping.early_stop == True or (epoch + 1) % save_iteration != 0:
            name = f"{self.cfg.model.model_flag}{self.cfg.nn.type}_{self.cfg.time}_{epoch+1}_{self.data_loader.training_shape}_{self.data_loader.training_col_shape}_{self.data_loader.validation_shape}_{self.cfg.dataset.transform_input}_{self.weight_data}_{self.weight_dt}_{self.weight_pinn}_{self.weight_pinn_ic}_{self.cfg.nn.weighting.update_weight_method}.pth"
            self.save_model(os.path.join(folder_name, name))
        self.final_name = os.path.join(folder_name, f"{self.cfg.model.model_flag}{self.cfg.nn.type}_{self.cfg.time}_{epoch+1}_{self.data_loader.training_shape}_{self.data_loader.training_col_shape}_{self.data_loader.validation_shape}_{self.cfg.dataset.transform_input}_{self.weight_data}_{self.weight_dt}_{self.weight_pinn}_{self.weight_pinn_ic}_{self.cfg.nn.weighting.update_weight_method}")
        total_test_loss =  self.test_model(0,500,wandb_run)
        return 
    ...
    def test_model(self, starting_traj=0, total_traj=1, run=None):
        ...
        Args:
            ...
            num_epochs (int): number of epochs
        """
        total_traj = total_traj if total_traj < 1000 else 10000 # fix it self.data_loader.total_test_trajectories
        sample_per_traj = 1000 # fix it
        ...
        self.log_plot(y_pred, y_test, None, run ,x_test) # True instead of run
    ...
    def plot_all(self, x_train, y_train):
        ...
        for i in range(y_train.shape[1]):
            ...
            plt.plot(x_train, y_train[:, i],  label='Original data')
            plt.plot(x_train, y_pred[:, i],  label='Fitted line')
            plt.legend()
            if i % 2 != 0:
    ...
    def plot_all_dt(self, x_train, y_train):
        ...
        dt = self.calculate_from_ode(y_train,x_train)  # Calculate the derivative of the output data
        dt_pred = self.calculate_from_ode(y_pred,x_train) # Calculate the derivative of the predicted output data
        ...
        for i in range(y_train.shape[1]):
            ...
            plt.plot(x_train, dt[:, i],  label='Original data')
            plt.plot(x_train, dt_pred[:, i],  label='Fitted line')
            plt.legend()
            if i % 2 != 0:
                plt.show()
        return