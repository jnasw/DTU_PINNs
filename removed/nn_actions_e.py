...
from src.nn.nn_dataset_e import DataSampler
...
class NeuralNetworkActions():
    ...
    def define_nn_model(self):
        ...
        for key in nn_cfg.keys():# find the correct nn config based on self.cfg.nn.type
            if key == self.cfg.nn.type:
                print(nn_cfg[key].items())
                for k, v in nn_cfg[key].items():
                    if k not in self.cfg.nn:
                        self.cfg.nn[k] = v
                    #else: # print 
                    #    print(f"Use existing values {v} for {k} found in self.cfg.nn")
                break
    ...
    #     """
        ...
        u_all = torch.cat(u, 1)
        if self.epoch_epoch == 1000:
            print("gamw2")
        return y, u_all
    
    def calculate_autograd22(self, x_train): #needs fix, cause the output is not in the same shame as it should
        """
        Calculates the model output and its derivatives efficiently.
        """
        
        time = x_train[:, 0].unsqueeze(1)
        no_time = x_train[:, 1:]
        y = self.forward_nn(time=time, no_time=no_time)

        # Efficiently calculate all derivatives at once (as before)
        grad_outputs = torch.ones_like(y).to(y.device) # Ensure on correct device
        
        u_all = torch.autograd.grad(y, time, grad_outputs=grad_outputs, create_graph=False, retain_graph=True)[0]
        return y, u_all
    ...
    def pinn_train2(self, num_of_skip_data_points, num_of_skip_col_points, num_of_skip_val_points, wandb_run=None):
        ...
        for epoch in range(self.cfg.nn.num_epochs):
            ...
            if self.cfg.nn.batch_size != "None":
                ...
                for (x_batch, y_batch), x_col_batch, (x_ic_batch, y_ic_batch) in zip(train_loader, collocation_loader, col_ic_loader):
                    ...
                    def closure():
                        ...
                        #print("output",output.shape, "dydt0",dydt0.shape, "ode0",ode0.shape)
                         # Efficiently calculate loss_dt and loss_pinn
            ...
            # log all the losses for the epoch to wandb 
            save_iteration = 100 if self.cfg.nn.optimizer == "LBFGS" else 10000 # 20 iterations within the optimizer ->500*20 = 10000
            if (epoch + 1) % save_iteration == 0:
