...
class DataSampler:
    ...
    def __init__(self, cfg):
        ...
        self.x, self.y  = self.data_input_target_limited2(self.data, self.time) # Convert the loaded data into input and target data with a time limit
    ...
    def data_input_target_limited2(self, data, time_limit):
        """
        ...
        for training_sample in data:
            ...
            y_train = training_sample_l[1:self.output_dim+1].T.clone().detach()
...
# import torch
    ...
    def data_input_target_limited(self, data, time_limit):
        ...
        for training_sample in data:
            ...
            training_sample_l = training_sample.T
            training_sample_l = training_sample_l[training_sample_l[:,0]<=time_limit].T # limit the time to time_limit
            y_train = training_sample_l[1:self.output_dim+1].T.detach()
            x_train = training_sample_l.T
            x_train[:,1:]=x_train[0][1:]
            x_train = x_train.clone().requires_grad_(True) # Use clone().requires_grad_(True)

            x_train_samples.append(x_train)
            y_train_samples.append(y_train)

        x_train_list = torch.cat(x_train_samples, 0) # Concatenate outside the loop
        y_train_list = torch.cat(y_train_samples, 0) # Concatenate outside the loop
        return x_train_list, y_train_list
